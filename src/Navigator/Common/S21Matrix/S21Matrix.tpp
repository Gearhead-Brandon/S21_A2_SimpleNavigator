...
S21Matrix<T>::S21Matrix(const S21Matrix &other)
  ...
  for (int i = 0; i < rows_; i++)
    for (int j = 0; j < cols_; j++) temp.matrix_[i][j] = other.matrix_[i][j];
...
void S21Matrix<T>::SetCols(int newCols) {
  if (newCols != cols_) Resize(rows_, newCols);
}
...
void S21Matrix<T>::SetRows(int newRows) {
  if (newRows != rows_) Resize(newRows, cols_);
}
